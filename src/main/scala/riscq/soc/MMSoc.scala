...
object MemMapReg {
  ...
  def pgFreq(i: Int) = pgBase(i) + 12
  def pgPhase(i: Int) = pgBase(i) + 16
  // def pgStart(i: Int) = pgBase(i) + 20

  def dcgBase(i: Int) = dcgOffset + dcgStep * i
  def dcgFreq(i: Int) = dcgBase(i)
  def dcgPhase(i: Int) = dcgBase(i) + 4
  ...
  def getCHeader(qubitNum: Int) = {
    ...
    for(i <- 0 until qubitNum * 3) {
      val base = pgOffset + pgStep * i
      res += s"#define DCG_${i}_FREQ 0x${dcgFreq(i).toHexString} \n"
      res += s"#define DCG_${i}_PHASE 0x${dcgPhase(i).toHexString} \n"
    }
...
case class MemMapRegFiber(pgs: List[pulse.PulseGenerator], dcgs: List[pulse.DemodCarrierGenerator], rds: List[pulse.ReadoutDecoder]) extends Area {
  val up = Node.up()
  ...
  val logic = Fiber build new Area {
    ...
    factory.write(startTime, startTimeAddr)
    ...
    for((pg, id) <- pgs.zipWithIndex) {
      ...
      pgFactory.driveFlow(getDriveReg(pg.io.addr), pgAddr(id))
      pgFactory.driveFlow(getDriveReg(pg.io.amp), pgAmp(id))
      pgFactory.driveFlow(getDriveReg(pg.io.dur), pgDur(id))
      pgFactory.driveFlow(getDriveReg(pg.io.freq), pgFreq(id))
      pgFactory.driveFlow(getDriveReg(pg.io.phase), pgPhase(id))
    }
    ...
    for((dcg, id) <- dcgs.zipWithIndex) yield new Area {
      ...
      dcgFactory.driveFlow(getDriveReg(dcg.io.freq), dcgFreq(id))
      dcgFactory.driveFlow(getDriveReg(dcg.io.phase), dcgPhase(id))
    }
    ...
    for((rd, id) <- rds.zipWithIndex) {
      ...
      // rd.io.cmd := cmdFlow
      // factory.driveFlow(getDriveReg(cmdFlow), rdDur(id))

      // val refRFlow = Reg(rd.io.refR)
      // rd.io.refR := refRFlow
      // factory.driveFlow(getDriveReg(refRFlow), rdRefR(id))

      // val refIFlow = Reg(rd.io.refI)
      // rd.io.refI := refIFlow
      // factory.driveFlow(getDriveReg(refIFlow), rdRefI(id))
      ...
      factory.driveFlow(getDriveReg(rd.io.dur), rdDur(id))
      factory.driveFlow(getDriveReg(rd.io.refR), rdRefR(id))
      factory.driveFlow(getDriveReg(rd.io.refI), rdRefI(id))
...
case class MemoryMapSoc(
  ...
  val dspArea = new ResetArea(riscqRst, false) {
    ...
    val pgs = List.fill(qubitNum * 2)(
      pulse.PulseGenerator(
        ...
        memLatency = 1 + pulseMems(0).withOutRegFast.toInt
        )
    )
  ...
  val rbArea = for(rbId <- 0 until qubitNum) yield new Area{
    val readoutBuffer = DualClockRam(width = 128, depth = 1024, slowCd = cd100m, fastCd = cd500m, withOutRegFast = true, withOutRegSlow = true)
